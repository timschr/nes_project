...
static void broadcast_distance_to_sink()
{
  ...
  for(e = list_head(neighbor_table); e != NULL; e = e->next) {
    if(linkaddr_cmp(from, &e->addr)) {
      ...
      if (rec_number_of_hops != NOT_INIT) {
        if (sink_hops - 1 > rec_number_of_hops || sink_hops == NOT_INIT) {
          sink_hops = rec_number_of_hops + 1;
          e->num_hops = rec_number_of_hops;

          broadcast_distance_to_sink();
        ...
        const linkaddr_t *prevhop, uint8_t hops)
{
  ...
  if (closest != NULL && closest->num_hops != NOT_INIT) {
